<tr
    data-add-member-url="{{ route('workshop::releases::addMember', ['release' => $release->getId()]) }}"
    data-remove-member-url="{{ route('workshop::releases::removeMember', ['release' => $release->getId()]) }}"
    data-approve-member-url="{{ route('workshop::releases::approveMember', ['release' => $release->getId()]) }}"
>
    <td class="work-status">
        <span class="label label-primary">Перакладаецца</span>
    </td>
    <td class="subject">
        <div class="table-content">
            ...
            <span class="label label-sm label-primary">Перакладаецца</span>
        </div>
    </td>
    <td class="work-progress">
        <div class="progress progress-xs table-content">
            <div class="progress-bar progress-bar-primary progress-bar-indicating" style="width: {{ $release->getReadiness() }}"
                 role="progressbar">
    ...
    <td class="members">
        <div class="table-content">
            <select multiple="multiple"
                    data-plugin="jquery-selective"
                    data-selectable-members=<?php echo json_encode($releaseMembers['selectable'][$release->getId()]) ?>
                            data-selective-members=<?php echo json_encode($releaseMembers['selected'][$release->getId()]) ?>
            ></select>
        </div>
    </td>
    <td class="actions">
